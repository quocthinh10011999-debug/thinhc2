...
import { 
  ...
  RefreshCw, Database, Trophy, BookOpen
} from 'lucide-react';
...
const AdminDashboard = () => {
  const { registrations, feedbacks, quizScores, quizSets, lastSync, isLoading, isApiConfigured } = useData();

  const recentRegs = registrations.slice(0, 3);
  const topScores = quizScores.slice(0, 3);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard label="Bộ đề nhận thức" value={quizSets.length} icon={BookOpen} color="bg-[#d4af37]" />
        <StatCard label="Lượt thi hoàn thành" value={quizScores.length} icon={Trophy} color="bg-green-500" />
        <StatCard label="Ý kiến đóng góp" value={feedbacks.length} icon={MessageSquare} color="bg-blue-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Quiz Top Scores Widget */}
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Bảng điểm Nhận thức mới nhất</h3>
              <Link to="/admin/quiz" className="text-[#800000] text-[10px] font-bold uppercase flex items-center hover:underline">
                Quản lý bộ đề <ArrowUpRight className="w-3 h-3 ml-1" />
              </Link>
            </div>
            <div className="space-y-4">
              {topScores.length > 0 ? topScores.map((score, i) => (
                <div key={score.id || i} className="flex items-center justify-between p-4 bg-slate-50 border-l-4 border-[#d4af37]">
                  <div className="flex items-center space-x-4">
                    <div className="w-8 h-8 rounded-full bg-[#d4af37] text-[#800000] flex items-center justify-center font-black text-[10px]">
                      {i + 1}
                    </div>
                    <div>
                      <p className="text-xs font-black text-slate-900 uppercase">{score.userName}</p>
                      <p className="text-[9px] text-slate-400 font-bold uppercase">{score.setTitle} • {score.unit}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-black text-[#800000]">{score.score}/{score.total}</p>
                    <p className="text-[8px] text-slate-400 uppercase font-black">{new Date(score.completedAt).toLocaleDateString('vi-VN')}</p>
                  </div>
                </div>
              )) : (
                <div className="text-center py-10 text-slate-400 font-bold uppercase text-[10px]">Chưa có dữ liệu thi</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="bg-[#800000] text-white rounded-xl shadow-xl p-8 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <Link to="/admin/quiz" className="block w-full text-center bg-[#d4af37] text-[#800000] py-3 text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 text-center space-y-4">
             ...
             <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tình hình học tập</h4>
             <p className="text-xs font-bold text-slate-600 uppercase">
                {quizScores.length > 0 ? `Đã có ${quizScores.length} chiến sĩ tham gia thi nhận thức chính trị trực tuyến.` : 'Chưa có dữ liệu thi trực tuyến.'}
             </p>
          </div>
        </div>
      </div>
